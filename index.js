...
var fileContent = "<Create award first>";
fs.writeFileSync('./latex/preview_URL.txt', fileContent);
...
//show preview URL
app.get("/user/awards/show", ensureLoggedIn("/login"), async (req, res) => {
  if (req.user.administrator) res.redirect("/login");
  const client = await pool.connect();
  var previewURL = fs.readFileSync('./latex/preview_URL.txt').toString();
  req.flash("info", previewURL);
  res.redirect("/user/awards/index");
  client.release();
});
